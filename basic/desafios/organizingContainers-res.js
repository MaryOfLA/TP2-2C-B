...
function organizingContainers(containers) {
    // Primero necesito saber la cantidad de contenedores.
    const cantContainers = containers.length;

    // Después la cantidad de bolas y tipos por container.
    const bolasPorContainer = new Array(cantContainers).fill(0);
    const tiposDeBolas = new Array(cantContainers).fill(0);

    // Recorro la matriz y sumo cada fila con las bolas en cada contenedor
    // Lo mismo con las columnas y los tipos
    for (let i = 0; i < cantContainers; i++) {
        const container = containers[i];
        for (let k = 0; k < cantContainers; k++) {
            const bola = container[k];
            bolasPorContainer[i] += bola;
            tiposDeBolas[k] += bola;
        }
    }

    // Necesito organizarlas para poder compararlas.
    const bolasOrganizadas = bolasPorContainer.sort();
    const tiposOrganizados = tiposDeBolas.sort();

    // Recorro y comparo.
    let indice = 0;
    while((indice < cantContainers) && (bolasOrganizadas[indice] === tiposOrganizados[indice])) {
        indice ++;
    }

    return indice === cantContainers ? POSSIBLE : IMPOSSIBLE;
}
